...
![Preview of my configuration](./screenshots/preview.png)

## Install Guide

To install the system, git is necessary. To install git temporarily, use the command `nix-shell -p git`

Clone the repository in your user directory with `git clone https://github.com/autumn-the-kitty-cat/public-nix-config`

In the home directory, create a `.config` directory with `mkdir ~/.config`

Copy the entire home-manager configuration to `.config` with the command `cp -r ~/public-nix-config/home-manager ~/config`

Copy the NixOS configuration with `sudo cp ~/public-nix-config/nixos/configuration.nix /etc/nixos` 

You can now remove the `nix-config` directory with `rm -rf ~/public-nix-config`
